	...
	public Item[] adicionaItens(double capacidade){
    	...
    	while(capacidade > 0 && itemIndice < this.itens.length){
    		if(this.itens[itemIndice].quantidade <= capacidade){
    			...
    			itensAdicionados[itemIndice] = new Item(this.itens[itemIndice].nome, this.itens[itemIndice].valor, this.itens[itemIndice].quantidade);
    		}
    		else{
    			// se não cabe inteiro, pegue a fração que puder
    			itensAdicionados[itemIndice] = new Item(this.itens[itemIndice].nome, this.itens[itemIndice].valor, capacidade);
    			capacidade = 0;
    		}
    		itemIndice++;
    	}
    	...
    	for(int i=itemIndice; i<this.itens.length; i++){
    		itensAdicionados[i] = new Item(this.itens[i].nome, this.itens[i].valor, 0.0);
    	}
